...
class ThetaCModelPlotter(BaseModelPlotter):
    ...
    def _extract_model_specific_samples(self, data: Dict[str, Any]):
        data["theta_c_samples"] = data["all_samples"][:, self.theta_c_idx]
        data["fj_samples"]      = data["all_samples"][:, self.fj_idx]
        # Pre-calculate epsilon to save time in loops
        # epsilon = f_j * (1 - cos(theta_c))
    ...
    def plot_violins_epsilon(self, filename="epsilon_violin_plot.pdf"):
        ...
        def label_gen(ax, x_end, y_map):
            for val, lab in zip([1e-4, 1], ["- $\\epsilon=10^{-4}$", "- $\\epsilon=1$"]):
                ax.text(x_end-0.5, y_map(np.log10(val)), lab, fontsize=12, va="center")
    ...
    def plot_violins_epsilon_(self, filename: str = "epsilon_violin_plot.pdf"):
        """
        Plot violin plots of epsilon distributions using O(1) lookup.
        """
        all_models = sorted(list(set(d["name"] for d in self.model_data)))
        ...
        for i_alpha, alpha in enumerate(self.alpha_keys):
            ...
            for model_name in all_models:
                ...
                rate_color = self.cmap(self.norm(data["rate"]))
    ...
    def plot_violins_fj_at_fixed_angles_(self, theta_c_values: List[float] = [5, 10, 20],
                                        filename: str = "fj_violin_plot_fixed_angles.pdf",
                                        max_fj: float = 10.0):
        """
        Plot violin plots of f_j distributions using O(1) lookup.
        """
        all_models = sorted(list(set(d["name"] for d in self.model_data)))
        ...
        # Pre-calc beam factors for the fixed angles
        # Map: index -> 1 - cos(theta)
        beam_denoms = [(1 - np.cos(np.deg2rad(th))) for th in theta_c_values]
        
        offsets = [(i - (len(theta_c_values) - 1) / 2) * 0.23 for i in range(len(theta_c_values))]
        hatch_patterns = ["///", "---", "xxx"][:len(theta_c_values)]
        min_log, max_log = -1, 1
        ...
        for i_alpha, alpha in enumerate(self.alpha_keys):
            ...
            for model_name in all_models:
                ...
                # Use pre-calc epsilon
                epsilon_samples = data["epsilon_samples"]

                # Iterate fixed angles
                for j, denom in enumerate(beam_denoms):
                    if denom <= 0: continue
        ...
        ax.set_ylabel(f"$\\alpha_{{\\mathrm{{CE}}}}$", fontsize=self.fontsize)
        ax.set_yticklabels([f"${a[1:]}$" for a in self.alpha_keys], fontsize=self.fontsize)
        ax.set_ylim(-0.1, len(self.alpha_keys) * y_step - 0.19)
