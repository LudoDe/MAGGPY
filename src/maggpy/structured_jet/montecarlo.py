...
from ..utils            import DEFAULT_LIMITS, luminosity_gen, compute_luminosity_distance, score_func_cvm
...
class SimParams:
    ...
    def __post_init__(self):
        ...
        self.geometric_factors      = 1 / (4 * np.pi * compute_luminosity_distance(self.z_arr)**2 * (1 - np.cos(self.theta_c)))
...
def generate_macro_properties(thetas : list, params : SimParams, interps :Interps, n_counts_new : int) -> dict:
    ...
    L_arr       = luminosity_gen(k_pl, n_counts_new, rng = rng) 
...
def score_func(y_sim, y_obs, rng=None):
    return score_func_cvm(y_sim, y_obs, rng=rng)
...
def calculate_isotropic_luminosity(
    ...
    D_L     = compute_luminosity_distance(z)
...
def make_observations_with_iso(
    ...
    E_iso_det, L_iso_det = calculate_isotropic_luminosity(obs)
