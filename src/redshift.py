...
def get_redshift_quantities(
    ...
    Parameters:
        mrd_path    : Path to the MRD data file
    ...
    if not mrd_path.exists(): raise FileNotFoundError(f"MRD file not found: {mrd_path}")
    ...
    return total_rate, local_rate, z_mrd, P_z_density
...
def sample_from_mrd(
    ...
    cdf_interp  = interp1d(cdf, z_grid, kind='linear', bounds_error=False, fill_value=(z_grid[0], z_grid[-1]))
