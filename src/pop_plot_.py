...
class ModelAnalyzer:
    ...
    def plot_comparison_total_rate_side_by_side(self, analyzer_bounded: 'ModelAnalyzer', figsize=(14, 6), f_max=10):
        ...
        #ax1.set_title("Bounded ($f_j \leq 1$)", fontsize=self.base_fontsize, pad=-100)
        
        # Plot current (unbounded) analyzer on the right
        plot_on_ax(ax2, self.all_data)
        self._format_axes(ax2, r'Total BNS Rate $\Lambda$ [yr$^{-1}$]')
        #move title inside the plot area
        #ax2.set_title("Unbounded", fontsize=self.base_fontsize, pad=-100)
        # use text instead of title for better positioning
...
class PopulationAnalysis:
    ...
    def plot_combined_analysis(self, figsize=(16, 12), f_max=5, fj_threshold=1.0):
        ...
        # R22 / L25 references
        for ax in [ax_med_loc, ax_quant_loc]:
            ax.set_xscale('log')
        for ax in [ax_med_tot, ax_quant_tot]:
            ax.set_xscale('log')
        ...
        # Unified Legend
        all_handles = [mpatches.Patch(color='gray', alpha=0.3, label='GWTC-4')] + \
                      list(alpha_handles.values()) + list(model_handles.values())
        
        fig.legend(handles=all_handles, loc='upper center', bbox_to_anchor=(0.5, 1.08), 
                   ncol=6, fontsize=self.base_fontsize - 6, frameon=False)
    ...
    def plot_fj_violins(self, filename="fj_violin_plots"):
        ...
        for i, alpha in enumerate(alphas):
            ...
            for j, model in enumerate(models):
                ...
                ax.text(j, y_base - 0.1, f"${med:.2f}_{{-{med-p05:.2f}}}^{{+{p95-med:.2f}}}$", 
                        fontsize=self.base_fontsize-12, ha='center', va='top')
        ...
        ax.set_ylim(-0.3, len(alphas) * y_step)
        
        # Colorbar
        sm = cm.ScalarMappable(cmap=cmap, norm=norm)
        sm.set_array([])
        cbar = fig.colorbar(sm, ax=ax, aspect=40)
        cbar.set_label(r'Local BNS Rate [Gpc$^{-3}$ yr$^{-1}$]', fontsize=self.base_fontsize)
    ...
    def plot_sgrb_rate_posteriors(self, filename="sgrb_rate_posteriors"):
        ...
        for d in self.data:
            ...
            # R_sGRB = R_BNS * f_j
            # Filter unphysical fj > 1 for this plot
            phys_fj = d["fj_samples"][d["fj_samples"] <= 1.0]
            if len(phys_fj) == 0: continue
            
            sgrb_rate = d["rate"] * phys_fj
            
            try:
                kde = gaussian_kde(np.log10(sgrb_rate))
                pdf = kde(np.log10(x_grid))
                ax.plot(x_grid, pdf, color=cm.viridis(0.5), alpha=0.3, lw=1) # Simplified coloring
