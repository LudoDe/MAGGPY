...
def flat_prior(thetas: Sequence[float]) -> float:
    """Independent flat prior over the six-parameter support."""
    theta = np.asarray(thetas, dtype=float)
    if theta.shape != (N_PARAMS,) or not np.all(np.isfinite(theta)):
        return -np.inf
...
def _make_initial_walkers(
    ...
    if n_walkers < 2 * N_PARAMS:
        raise ValueError(f"emcee requires at least {2 * N_PARAMS} walkers for {N_PARAMS} parameters")
    ...
    if center.shape != (N_PARAMS,) or scale.shape != (N_PARAMS,):
        raise ValueError(f"initial_center and initial_scale must each have {N_PARAMS} entries")
    if not np.all(scale > 0):
        raise ValueError("Every initial_scale entry must be positive")
    if not np.isfinite(flat_prior(center)):
        raise ValueError("initial_center must lie strictly inside the flat-prior bounds")
