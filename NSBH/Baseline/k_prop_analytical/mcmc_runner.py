...
def build_cvm_lookups(
    ...
    fixed_geometry=(10.0, 5.0, 25.0), # (theta_c_bns, fj_bns, theta_c_nsbh)
    n_l0=100, n_k=50
):
    """
    ...
    for i, l0 in enumerate(l0_grid):
        for j, l0_k in enumerate(k_grid):
            ...
            thetas = [l0, l0_k, theta_c_bns, fj_bns, theta_c_nsbh]
...
def forward_model_fast(
    ...
    L_L0, k_nsbh_log, theta_c_bns, fj_bns, theta_c_nsbh = thetas
...
def plot_corner_cut(alphas, burn_frac: float = 0.33, thin: int = 10, fj_cut = 1.0, only_physical = False):
    ...
    for alpha in alphas:
        ...
        fj_index    = 3
        mask        = flat[:, fj_index] < fj_cut #! careful index has changed due to removal of L_mu_E and sigma_E
        flat_leq    = flat[mask]
        flat_geq    = flat[~mask] 
        color_leq   = "k"
        color_geq   = "red"
        ...
        if only_physical: corner_args["range"][fj_index] = (0, 1)
        ...
        if not only_physical:
            #change range of the fj value to 0-1
